...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
	...
	public Object applyBeanPostProcessorsAfterInitialization(Object existingBean, String beanName)
		...
		Object result = existingBean;
		// 通过AnotationAwareAspectAutoProxyCreator 变成代理对象
		for (BeanPostProcessor processor : getBeanPostProcessors()) {
			// 干扰spring 的bean的初始化
	...
	protected Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		try {
			...
			//第一次应用spring的后置处理器
			//在bean 初始化前应用后置处理，如果后置处理返回的bean 不为空，则直接返回
	...
	protected Object doCreateBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
		...
		if (instanceWrapper == null) {
			// new对象, 第二次调用后置处理器,对象的创建,会对象执行构造方法
		...
		/**
		 * 循环依赖重点:
		 * 判断是否循环依赖 allowCircularReferences 默认为true 所以spirng 默认支持循环依赖
		 * 这里判断是不是支持循环引用和是否单例以及bean是否在创建过程中
		 * 判断循环引用的是&& this.allowCircularReferences
		 * allowCircularReferences在spring 源码中默认是true
		 * Eagerly cache singletons to be able to resolve circular references
		 */
		boolean earlySingletonExposure = (mbd.isSingleton() && this.allowCircularReferences &&
				isSingletonCurrentlyInCreation(beanName));

		// 如果是单例, 并且正在创建, 并且允许循环依赖 , 这里是spring的原型是不支持循环依赖的证据
		if (earlySingletonExposure) {
			if (logger.isTraceEnabled()) {
				logger.trace("Eagerly caching bean '" + beanName +
						"' to allow for resolving potential circular references");
			}
			//第四次调用后置处理器, 判断是否需要AOP , index-->user ---->index为了解决无限循环,把index放入缓存之中+
			// 放入的不是一个对象,而是
	...
	protected BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
		...
		// Candidate constructors for autowiring? 反射实例化对象
		...
		if (hasInstAwareBpps) {
			...
			for (BeanPostProcessor bp : getBeanPostProcessors()) {
				if (bp instanceof InstantiationAwareBeanPostProcessor) {
					InstantiationAwareBeanPostProcessor ibp = (InstantiationAwareBeanPostProcessor) bp;
					/**
					 * 处理@Resource 和 处理@Autowired的 后置处理器不一样
					 * CommonAnnotationBeanPostProcessor 是处理@Resource的
					 * AutowiredAnnotationBeanPostProcessor 是处理Autowired的
					 */
		...
		for (String propertyName : propertyNames) {
			try {
				...
				if (Object.class != pd.getPropertyType()) {
					...
					// 根据容器的Bean 定义解析依赖关系, 返回所以要被注入的Bean对象
					Object autowiredArgument = resolveDependency(desc, beanName, autowiredBeanNames, converter);
					if (autowiredArgument != null) {
						pvs.add(propertyName, autowiredArgument);
					}
					for (String autowiredBeanName : autowiredBeanNames) {
						// 指定名称和属性注册依赖Bean的名称, 进行属性依赖注入
	...
	protected void applyPropertyValues(String beanName, BeanDefinition mbd, BeanWrapper bw, PropertyValues pvs) {
		...
		for (PropertyValue pv : original) {
			if (pv.isConverted()) {
				...
				// 转换属性值, 例如将引用转换为IOC容器中的实例化对象引用 ,类型强转
